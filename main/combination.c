...
// ================= FLEX SENSOR CONFIG =================
#define SAMPLES 20 // Averaging samples
#define FLEX1 ADC_CHANNEL_0  // GPIO36 (Thumb)
...
// ================= TAG =================
static const char *TAG = "FLEX+MPU+DFPLAYER";
...
void play_mp3_file(int file_number) {
    ...
    df_send(CMD_PLAY_TRACK, file_number);
}
...
void app_main(void) {
    ...
    adc_oneshot_chan_cfg_t adc_config = { .atten = ADC_ATTEN_DB_11, .bitwidth = ADC_BITWIDTH_12 };
    adc_oneshot_config_channel(adc1_handle, FLEX1, &adc_config);
    adc_oneshot_config_channel(adc1_handle, FLEX2, &adc_config);
    adc_oneshot_config_channel(adc1_handle, FLEX3, &adc_config);
    adc_oneshot_config_channel(adc1_handle, FLEX4, &adc_config);
    adc_oneshot_config_channel(adc1_handle, FLEX5, &adc_config);
    ...
    i2c_write(PWR_MGMT_1, 0); // Wake MPU6050
    ...
    while (1) {
        ...
        // ==== FLEX + COMBO CONDITIONS ====
        if (is_in_trigger_range(index) && is_in_trigger_range(middle) && last_played != 7) {
            play_mp3_file(7); last_played = 7;
        }
        else if (is_in_trigger_range(middle) && is_in_trigger_range(ring) && last_played != 8) {
            play_mp3_file(8); last_played = 8;
        }
        else if (is_in_trigger_range(middle) && gyro_z > 15000 && last_played != 9) {
            play_mp3_file(9); last_played = 9; // Middle + Right turn
        }
        else if (is_in_trigger_range(middle) && gyro_z < -15000 && last_played != 10) {
            play_mp3_file(10); last_played = 10; // Middle + Left turn
        }
        else if (is_in_trigger_range(index) && last_played != 1) { play_mp3_file(1); last_played = 1; }
        else if (is_in_trigger_range(thumb) && last_played != 2) { play_mp3_file(2); last_played = 2; }
        ...
        else if (!is_in_trigger_range(thumb) && !is_in_trigger_range(index) &&
                 !is_in_trigger_range(middle) && !is_in_trigger_range(ring) &&
                 !is_in_trigger_range(pinky)) {
            last_played = 0; // Reset when nothing active
        }
